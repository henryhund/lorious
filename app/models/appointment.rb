class Appointment < ActiveRecord::Base
  ...
  scope :pending, -> { where("appt_state <> 'cancelled' AND time > ? AND (expert_confirmed = false OR user_confirmed = false OR expert_confirmed is NULL OR user_confirmed is NULL)", Time.now) }
  scope :upcoming, -> { where("expert_confirmed = true AND user_confirmed = true AND time + interval '1' minute * duration >= ?", Time.now )  }
