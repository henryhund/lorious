...
app = angular.module("search", ["ngResource"]).config([
  "$routeProvider", "$httpProvider", function($routeProvider, $httpProvider) {
    return $routeProvider.when("/", {
      templateUrl: '/assets/search-list.html',
...
this.SearchCtrl = [
  "$scope", "$resource", "$route", "$routeParams", "$location", "Search", function($scope, $resource, $route, $routeParams, $location, Search) {
    $scope.pg_counter = 1;
    $scope.experts = Search.query({id: $scope.pg_counter++});
    $scope.load_data = function() {
	  $scope.new_experts = Search.query({id: $scope.pg_counter++}, function (new_experts) {
        //new data is available in the callback to insert to the experts ng-model
        $scope.experts.push.apply($scope.experts, new_experts);
      });
...
(function() {
  $(function() {
    return $("body").on("click", ".content-load", function(event) {
      var content_handler, element, rest_link;
      element = $(event.target);
      rest_link = element.data("content-url");
      content_handler = $(element.data("content-handler"));
      return $.ajax(rest_link, {
        success: function(response) {
          return content_handler.html(response);
        }
      });
    });
  });
